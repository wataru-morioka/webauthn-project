...
var (
	accountDBTypes = map[string]string{`ID`: `integer`, `IsDeleted`: `boolean`, `IsAdmin`: `boolean`, `Account`: `character varying`, `Name`: `character varying`, `State`: `character varying`, `LoginCount`: `integer`, `LatestLoginAt`: `timestamp without time zone`, `CreatedAt`: `timestamp without time zone`, `ModifiedAt`: `timestamp without time zone`, `UID`: `character varying`, `Thumbnail`: `bytea`}
	_              = bytes.MinRead
)
